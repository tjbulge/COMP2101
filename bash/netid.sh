...
#if $1 in $interfacelist

#grabbing the list of interfaces from the ip link show command 
...
while [ $# -gt 0 ]; do
   ...
   * )
      echo "$1 is not a recognized argument... exiting."
      exit 2
      ;;
...
elif [ "$verbose" = "" ] && [ "$interfaceEntered" = "yes" ]; then
  echo ""
