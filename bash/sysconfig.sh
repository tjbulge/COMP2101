...
Displays complete system information, can display partial information using one or more of the following arguments:

-h | --help	Displays help message

--host		hostname information

--domain	Shows which domain the device is a part of

--ipconfig 	IP address info

--os		information about the current operating system (version, type, etc)

--cpu		installed CPU name, speed and manufacturer as well as BIOS information

--memory	amount of available memory and usage

--disk		available disk space and usage

--printer	default printer and other connected printers
...
Cleaning up temporary files and exiting program...

EOF

rm /tmp/sysreport.* 2>/tmp/rmerror
rm /tmp/memoryinfo.* 2>/tmp/rmerror
rm /tmp/cpuinfo.* 2>/tmp/rmerror
rm /tmp/businfo.* 2>/tmp/rmerror
rm /tmp/sysinfo.* 2>/tmp/rmerror
